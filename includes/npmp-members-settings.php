...
function npmp_render_members_page() {
    ...
    if (!empty($_GET['action']) && $_GET['action'] === 'edit' && !empty($_GET['id']) && current_user_can('manage_options')) {
        ...
        echo '</select></td></tr>';
        echo '</table>';
        submit_button('Save Changes', 'primary', 'npmp_save_member');
        echo '</form>';
        echo '</div>';
        return;
    }
    ...
    $form_submitted = (!empty($_POST['manual_add']) || !empty($_POST['npmp_subscribe'])) && isset($_POST['npmp_members_nonce']) && 
        wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['npmp_members_nonce'])), 'npmp_manage_members');
    ...
    foreach ($members as $member) {
        ...
        echo '<td><a href="?page=npmp_members&action=edit&id=' . esc_attr($member->id) . '">Edit</a> | ';
        echo '<a href="' . esc_url(wp_nonce_url('?page=npmp_members&action=delete&id=' . $member->id, 'npmp_manage_members_delete_' . $member->id)) . '" onclick="return confirm(\'Are you sure?\')">Delete</a></td>';
        echo '</tr>';
    }
