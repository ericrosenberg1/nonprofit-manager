...
function np_render_members_page() {
    ...
    // Handle deletion
    if (!empty($_GET['action']) && $_GET['action'] === 'delete' && !empty($_GET['id']) && check_admin_referer('np_manage_members_delete_' . intval($_GET['id']))) {
    ...
    $form_submitted = (!empty($_POST['manual_add']) || !empty($_POST['np_subscribe'])) && isset($_POST['np_members_nonce']) && wp_verify_nonce(wp_unslash($_POST['np_members_nonce']), 'np_manage_members');
